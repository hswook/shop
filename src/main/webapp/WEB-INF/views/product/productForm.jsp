...
<h1>
	...
	<form id="productForm" name="productForm" method="post" enctype="multipart/form-data">
		<fieldset>
			...
			<span>name</span><input name="name" type="text" value="${product.name }"><br />
			<span>main image</span><input name="mainImg" type="file" value="${product.mainImg }"><br /> 
			<span>price</span><input name="price" type="text" value="${product.price }"><br />
			<select name="type">
				<option value="t-shirt" <c:if test='${product.type eq "t-shirt" }'>checked</c:if> > t-shirt</option>
				<option value="outer" <c:if test='${product.type eq "outer" }'>checked</c:if> >outer</option>
				<option value="knit" <c:if test='${product.type eq "knit" }'>checked</c:if> >knit</option>
				<option value="food" <c:if test='${product.type eq "food" }'>checked</c:if> >food</option>
			</select><br />
